...
const getPersonioConfig = () => ({
  subdomain: process.env.PERSONIO_COMPANY_SUBDOMAIN || 'attolabs',
...
app.get('/api/jobs', async (c) => {
  const { subdomain } = getPersonioConfig()
  try {
...
app.post('/api/apply', async (c) => {
  const { companyId, accessToken, recruitingChannelId } = getPersonioConfig()
