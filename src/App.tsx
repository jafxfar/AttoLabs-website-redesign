...
function PageHero({
  ...
  const reduce = useReducedMotion()
  return (
    <section className={`relative overflow-hidden py-20 md:py-28 ${tokens.heroClass}`}>
      {!tokens.ink && <div className="coral-grid absolute inset-0 opacity-20" aria-hidden="true" />}
      <div className="site-wrap relative">
        ...
        >
          <SectionTag light={!tokens.ink}>{tag}</SectionTag>
        </motion.div>
        <motion.h1
          className="display-font mt-8 max-w-4xl text-[clamp(2.8rem,6.3vw,6.1rem)] font-semibold leading-[.91] tracking-[-.075em]"
        ...
        {children && (
          <motion.div
            className="mt-8 max-w-xl text-lg leading-[1.65]"
...
function Stats() {
  return (
    <section className="bg-[#1A1A1A] text-[#F4ECE7]">
      <Stagger className="site-wrap grid grid-cols-2 md:grid-cols-4">
        {[['04', 'countries'], ['07', 'industries'], ['06', 'services'], ['∞', 'possibilities']].map(([num, label], i) => (
          <StaggerItem
            key={label}
...
function Process({ sectionTheme = 'yellow' }: { sectionTheme?: SectionTheme }) {
  const steps = [
    ...
    ['03', 'Build in the open', 'Small releases, honest trade-offs, senior hands on the work. No black boxes.'],
    ['04', 'Leave it stronger', 'Your team gets a product, a platform, and the confidence to keep moving.'],
  ]
    ...
    <section className={`py-24 md:py-32 ${tokens.heroClass}`}>
      <div className="site-wrap">
        <Reveal>
          ...
          >
            <span>Technology moves quickly. Our job is to bring enough rigour to make that speed useful.</span>
          </SectionIntro>
        </Reveal>
        <Stagger className={`mt-16 grid border-t md:grid-cols-4 ${borderClass}`}>
...
function Contact({ sectionTheme = 'coral' }: { sectionTheme?: SectionTheme }) {
  ...
  return (
    <section
      id="contact"
      className={`relative overflow-hidden py-24 md:py-36 ${tokens.heroClass}`}
    >
      {!tokens.ink && <div className="coral-grid absolute inset-0 opacity-25" aria-hidden="true" />}
      <div className="site-wrap relative grid gap-14 md:grid-cols-[1fr_.8fr] md:gap-24">
        <Reveal>
      ...
      <PageFrame>
        <main>
          <PageHero
            theme={theme}
            tag="01 / All work"
            title={<>Projects with<br /><span style={{ color: themeTokens[theme].accent }}>a point of view.</span></>}
          >
...
function CooperationPage({ locale = 'en' }: { locale?: Locale }) {
  const theme = pageThemes.cooperation
  return (
    <>
      ...
      <PageFrame>
        <main>
          <PageHero
            theme={theme}
            tag="01 / Work with us"
            title={<>Start with the<br /><span style={{ color: themeTokens[theme].accent }}>hard problem.</span></>}
          >
            Tell us what is changing, what is stuck, and what a useful outcome would look like. We will answer with a concrete approach.
          </PageHero>
          <Process sectionTheme={theme} />
          <Contact sectionTheme={theme} />
        </main>
      </PageFrame>
    </>
  )
...
function CareersPage({ locale = 'en' }: { locale?: Locale }) {
  const theme = pageThemes.careers
  return (
    <>
      ...
      <PageFrame>
        <main>
          <PageHero
            theme={theme}
            tag="01 / Careers"
            title={<>Build the<br /><span style={{ color: themeTokens[theme].accent }}>next thing.</span></>}
          >
